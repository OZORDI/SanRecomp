    ...
    xpointer<be<uint16_t>>* pwszButtons, uint32_t dwFocusButton, uint32_t dwFlags, be<uint32_t>* pResult, XXOVERLAPPED* pOverlapped)
{
    *pResult = cButtons ? cButtons - 1 : 0;
    ...
    wprintf(L"[XamShowMessageBoxUI] please create an issue at https://github.com/OZORDI/SanRecomp/issues.\n");
    wprintf(L"[XamShowMessageBoxUI] !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
...
// Stub for achievement enumeration - GTA V specific
...
uint32_t XamInputGetState(uint32_t userIndex, uint32_t flags, XAMINPUT_STATE* state)
{
    ...
    {
        // GTA V Keyboard Input - Movement (WASD → Left Stick)
        ...
        // GTA V - Right stick handled by mouse camera (see below)
        ...
        // GTA V - Triggers handled by mouse buttons (see below)
        ...
        // GTA V - Shoulders (LB=Action/E, RB=Cover/Q)
        ...
        // GTA V - Face Buttons (A=Sprint/Shift, B=Crouch/Ctrl, X=Jump/Space, Y=Enter Vehicle/F)
        ...
        // GTA V Additional Keys - Map to appropriate buttons
        ...
        // GTA V Mouse Input (camera and actions)
        if (hid::g_inputDevice == hid::EInputDevice::Mouse || MouseCamera::IsActive())
        {
            // GTA V Mouse Buttons → Triggers/Buttons
            ...
            // GTA V Mouse Wheel → Weapon Switching (D-Pad Left/Right)
...
// GTA V uses these for player preferences, stats, and settings
