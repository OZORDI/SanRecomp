...
static const std::string DLCValidationFile = "default.xex";  // GTA V DLC uses xex as validation
...
// GTA V RPF archives that need extraction
...
static std::filesystem::path extractZipToTemp(const std::filesystem::path &zipPath)
{
    ...
    {
        g_tempExtractDir = std::filesystem::temp_directory_path() / "SanRecomp_DLC";
    }
...
{
    // GTA V DLC detection - check for specific files
    ...
    journal.lastErrorMessage = fmt::format("Could not detect GTA V DLC type for {}.", sourcePath.string());
    return DLC::Unknown;
}
...
bool Installer::parseSources(const Input &input, Journal &journal, Sources &sources)
{
    ...
    // Parse the contents of the title update (GTA V uses full XEX replacement).
...
bool Installer::install(const Sources &sources, const std::filesystem::path &targetDirectory, bool skipHashChecks, Journal &journal, std::chrono::seconds endWaitTime, const std::function<bool()> &progressCallback)
{
    ...
    // GTA V title updates are full XEX replacements (not .xexp patches)
    ...
    // Extract RPF archives (GTA V specific)
...
{
    // GTA V title updates contain a full XEX replacement (not .xexp patch)
    std::unique_ptr<VirtualFileSystem> sourceVfs = createFileSystemFromPath(sourcePath);
    if (sourceVfs == nullptr)
    {
        return false;
