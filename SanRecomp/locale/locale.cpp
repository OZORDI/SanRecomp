...
std::unordered_map<std::string_view, std::unordered_map<ELanguage, std::string>> g_locale =
{
    ...
    },
    {
        ...
        {
            { ELanguage::English,  "The module \"%s\" could not be found.\n\nPlease make sure that:\n\n- You extracted this copy of San Recompiled fully and not just the *.exe file.\n- You are not running San Recompiled from a *.zip file." },
            { ELanguage::Japanese, "モジュール\"%s\"が見つかりませんでした\n\n次の点を確認してください：\n\n※San Recompiledの*.exeファイルだけを抽出していなく、 コピーを完全に抽出してること\n※San Recompiledを*.zipファイルから実行していないこと" },
            { ELanguage::German,   "Das Modul \"%s\" konnte nicht gefunden werden.\n\nBitte stelle sicher, dass:\n\n- Diese Kopie von San Recompiled vollständig entpackt wurde und nicht nur die *.exe-Datei.\n- San Recompiled nicht direkt aus einer *.zip-Datei ausgeführt wird." },
            { ELanguage::French,   "Le module \"%s\" n'a pas pu être trouvé.\n\nVeuillez vous assurer que :\n\n- Vous avez extrait San Recompiled dans son entièreté et pas seulement le fichier *.exe.\n- Vous n'exécutez pas San Recompiled à partir d'un fichier *.zip." },
            { ELanguage::Spanish,  "No se pudo encontrar el módulo \"%s\".\n\nAsegúrese de que:\n\n- Ha extraido esta copia de San Recompiled por completo y no solo el archivo *.exe.\n- No está ejecutando San Recompiled desde un archivo *.zip." },
            { ELanguage::Italian,  "Impossibile trovare il modulo \"%s\".\n\nAssicurati che:\n\n- Hai estratto questa copia di San Recompiled correttamente e non solo il file *.exe.\n- Non stai eseguendo San Recompiled da un file *.zip." }
        }
    },
    {
