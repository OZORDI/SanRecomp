...
namespace VFS
{
    ...
    // GTA V file extensions to try when path doesn't include extension
    ...
    void Initialize(const std::filesystem::path& extractedRoot)
    {
        ...
        // Set up default path mappings for GTA V
    ...
    std::filesystem::path Resolve(const std::string& guestPath)
    {
        ...
        // Try with GTA V extensions if path has no extension
    ...
    void ResetPathMappings()
    {
        ...
        // GTA V uses "platform:/textures/fonts" etc. for Xbox 360 platform assets
